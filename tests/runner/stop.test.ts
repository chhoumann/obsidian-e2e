...
import {
  ...
  stopInstance,
} from "../../src/runner/stop";
import { cleanupTempDirectories, createTempDir } from "../helpers/create-temp-dir";

// stop.ts is plugin-agnostic: the marker filename is passed in, so the suite
// picks one representative slug/marker and exercises every path against it.
const MARKER_FILE = "podnotes-e2e-instance.json";
...
// a sibling worktree, another plugin's instance) use different user-data-dirs.
const PS_FIXTURE = [
...
async function seedInstance(profileRoot: string, id: string, vaultPath: string): Promise<string> {
  const udd = path.join(profileRoot, id, "home", "Library", "Application Support", "obsidian");
...
describe("commandMatchesInstance", () => {
  ...
  it("does not match the dev vault, another plugin, or a sibling worktree", () => {
    expect(
      commandMatchesInstance(
  ...
  it("does not match a process that mentions the path outside the --user-data-dir flag", () => {
    ...
    expect(commandMatchesInstance(`tail -f ${INSTANCE}/home/obsidian.log`, INSTANCE)).toBe(false);
  });
});
...
describe("parsePsOutput / collectInstancePids", () => {
  ...
  it("collects the instance tree (incl. token-less descendants) and nothing else", () => {
    ...
    // only by the descendant walk). 200/201 dev vault, 300 another plugin, 400
    // sibling worktree are all excluded.
...
describe("stopInstance", () => {
  ...
  it("preserves the profile when a liveness probe fails unexpectedly", async () => {
    // Critical regression: an unexpected kill(pid, 0) failure (EIO) must NOT be
    // read as "dead" - if it were, we would drop the pid from survivors, skip its
    // SIGKILL, and rm the profile of a possibly-live process. The probe rethrows,
    // so stopInstance aborts before removeDir.
    let removeCalled = false;
    const kill = (_pid: number, signal: string | number) => {
      if (signal === 0) {
  ...
  it("rechecks liveness at least once before SIGKILL even with a zero grace window", async () => {
    // Grace-loop regression: with a tiny/zero graceMs the deadline is already in
    // the past on the first check. A `while` loop would skip the body entirely and
    // SIGKILL every seeded pid; the `do/while` guarantees one post-SIGTERM recheck,
    // so a pid that exits right after SIGTERM is spared the SIGKILL.
    const { calls, kill } = makeKillSpy({ alive: false });
    const result = await stopInstance(INSTANCE, {
      runPs,
      kill,
    ...
    expect(calls.some((c) => c.signal === 0)).toBe(true);
    expect(calls.some((c) => c.signal === "SIGKILL")).toBe(false);
...
describe("orphan detection", () => {
  it("reads the registered vault paths", async () => {
    const root = await createTempDir(tempDirectories, "podnotes-profiles-");
  ...
  it("is orphaned only when every backing vault is gone", async () => {
    const root = await createTempDir(tempDirectories, "podnotes-profiles-");
    const liveVault = await createTempDir(tempDirectories, "live-vault-");
    ...
    await expect(isInstanceOrphaned(live, MARKER_FILE)).resolves.toBe(false);
    await expect(isInstanceOrphaned(gone, MARKER_FILE)).resolves.toBe(true);
  });
  ...
  it("prefers the worktree marker: worktree-gone reaps even with a live vault; worktree-alive spares even with a missing vault", async () => {
    const root = await createTempDir(tempDirectories, "podnotes-profiles-");
    const liveWorktree = await createTempDir(tempDirectories, "live-worktree-");
    ...
    await fs.writeFile(
      path.join(leaked, MARKER_FILE),
      JSON.stringify({ worktreePath: "/worktree/removed/on/merge" }),
    );

    await expect(isInstanceOrphaned(spared, MARKER_FILE)).resolves.toBe(false);
    await expect(isInstanceOrphaned(leaked, MARKER_FILE)).resolves.toBe(true);
  });
  ...
  it("stays conservative when the registration is unreadable", async () => {
    ...
    await expect(isInstanceOrphaned(bare, MARKER_FILE)).resolves.toBe(false);
  });

  it("rejects a marker whose JSON value is not an object", async () => {
    await expect(
      readInstanceMarker(INSTANCE, MARKER_FILE, { readFile: async () => '"not-a-marker"' }),
    ).resolves.toBeNull();
  });

  it("rejects an array-shaped marker", async () => {
    await expect(
      readInstanceMarker(INSTANCE, MARKER_FILE, { readFile: async () => "[]" }),
    ).resolves.toBeNull();
  });
});
...
describe("reapOrphanedInstances", () => {
  ...
  it("returns an empty result when the profile root does not exist", async () => {
    await expect(
      reapOrphanedInstances({ profileRoot: "/no/such/profile/root", markerFile: MARKER_FILE }),
    ).resolves.toEqual({ scanned: 0, reaped: [] });
  });

  it("refuses to scan or remove through a symlinked (insecure) profile root", async () => {
    // Temp-squat guard: a world-writable /tmp profile root could be swapped for a
    // symlink to an attacker dir. reapOrphanedInstances must not readdir/rm through
    // it - it reports the refusal and reaps nothing, even though the target holds a
    // reapable orphan.
    const root = await createTempDir(tempDirectories, "reap-insecure-");
    const target = path.join(root, "attacker-owned");
    await seedInstance(target, "podnotes-gone-cccccccccccc", "/does/not/exist");
    ...
    expect(result).toEqual({ scanned: 0, reaped: [] });
    expect(removed).toEqual([]);
    expect(logs.some((m) => m.includes("insecure profile root"))).toBe(true);
  });
});
